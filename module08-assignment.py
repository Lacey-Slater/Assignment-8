...
#"C001": {"company_name": "TechFlow Inc", "contact_person": "Alice Chen", "email": "alice@techflow.com", "phone": "555-0101"},
#"C002": {"company_name": "Techgridy Solutions", "contact_person": "Randy Marsh", "email": "randy@tegridy.org", "phone": "555-1002"},
#"C003": {"company_name": "Pear Tech Inc.", "contact_person": "Cim Took", "email": "Cim@notapple.com", "phone": "555-1003"},
#"C004": {"company_name": "Vishkar Corp", "contact_person": "Vaira Singhania", "email": "domina@vishkar.org", "phone": "555-1004"}
...
for customer_id, info in customers.items():
    print(f"ID: {customer_id} | Co: {info['company_name']} | Contact: {info['contact_person']}")
...
projects = {
    "C001": [
        {"name": "Customer Portal", "service": "Web Development", "hours": 50, "budget": 15000, "status": "completed"},
        {"name": "Data Handling", "service": "Data Analysis", "hours": 20, "budget": 5000, "status": "active"}
    ],
    "C002": [{"name": "IT Support Contract Sale", "service": "Customer Service", "hours": 2, "budget": 300, "status": "active"}],
    "C003": [{"name": "Company Wide Crash", "service": "IT Support", "hours": 30, "budget": 10000, "status": "pending"}],
    "C004": [{"name": "Secure Communications Setup", "service": "Web Development", "hours": 100, "budget": 30000, "status": "pending"}] 
}
...
print("-" * 60)
# Add your code here
for client_id, project_list in projects.items():
    for project in project_list:
        rate = services.get(project["service"], 0)
        cost = rate * project["hours"]
        print(f"Project: {project['name']} | Calculated Cost: ${cost}")
...
print(f"Company Names: {[customer['company_name'] for customer in customers.values()]}")
...
all_projects = [project for sublist in projects.values() for project in sublist]
...
print(f"Total Budget: ${total_budget} | Avg: ${avg_budget:.2f} | Max: ${max_budget}")
...
for customer_id, info in customers.items():
    ...
    project_hours = sum(project["hours"] for project in customer_projects)
    project_budget = sum(project["budget"] for project in customer_projects)
    print(f"{info['company_name']} ({customer_id}): Projects: {project_count}, Hours: {project_hours}, Budget: ${project_budget}")
...
# Add your code here
adjusted_rates = {service: round(rate * 1.1, 2) for service, rate in services.items()}
...
print("-" * 60)
# Add your code here
active_customers = {customer_id: customers[customer_id] for customer_id, project_list in projects.items() if len(project_list) > 0}
...
print("-" * 60)
# Add your code here
customer_budgets = {customer_id: sum(project["budget"] for project in projects) for customer_id, projects in projects.items()}
...
for customer_id, info in customers.items():
    print(f"ID {customer_id} Valid: {validate_customer(info)}")
...
def analyze_customer_budgets(projects_dict):
    stats = {}
    for customer_id, projects in projects_dict.items():
        if not projects:
            stats[customer_id] = {"total": 0, "average": 0, "count": 0}
            continue
        total = sum(project["budget"] for project in projects)
        stats[customer_id] = {"total": total, "average": total / len(projects), "count": len(projects)}
    return stats
